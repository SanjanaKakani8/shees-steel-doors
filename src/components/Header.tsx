...
export default function Header() {
  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `px-3 py-1.5 text-[11px] uppercase tracking-widest font-display transition-all duration-200 border-b whitespace-nowrap ${
      isActive
  ...
  const mobileLinkClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center justify-between w-full px-4 py-3 rounded-none text-left text-xs uppercase tracking-wider font-display font-medium transition ${
      isActive
    ...
    <header id="header-nav" className="sticky top-0 z-50 bg-[#FAF9F6]/95 backdrop-blur-md border-b border-stone-200/80 text-stone-900 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-18">
          ...
          {/* Logo & Brand Identity */}
          <NavLink to="/" className="flex items-center cursor-pointer shrink-0">
            <SheesLogo size="sm" />
          </NavLink>
          ...
          <div className="hidden sm:flex items-center gap-1.5 shrink-0">
            ...
            <a
              ...
              href={`https://wa.me/91${BUSINESS_INFO.whatsapp}?text=${encodeURIComponent('Hi, I visited your website and want to inquire about SHEES Steel Doors.')}`}
              target="_blank"
        ...
        <div className="lg:hidden bg-[#FAF9F6] border-t border-stone-200/80 px-4 py-4 space-y-4 shadow-xl">
          ...
          <div className="grid grid-cols-2 gap-2 pt-3 border-t border-stone-200/60">
            ...
            <a
              ...
              href={`https://wa.me/91${BUSINESS_INFO.whatsapp}?text=${encodeURIComponent('Hi, I want to inquire about your steel doors.')}`}
              target="_blank"
              referrerPolicy="no-referrer"
